...
**Status:** ✅ Done — implemented in `internal/git/rename.go`
...
**Status:** ✅ Done — implemented in `internal/ignore/`, supports `--ignore` flag and `.hcignore` files
...
**Status:** ✅ Done — implemented in `internal/git/decay.go`, exposed via `--decay`/`-D` flag
...
**Status:** Partial — indentation-based complexity implemented (`--indentation`/`-i`), cyclomatic complexity not yet started
