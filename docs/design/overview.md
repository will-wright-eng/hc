...
hc analyze --coupling                 # add change coupling pairs to the JSON envelope (requires --json)
...
### Change Coupling (`--coupling`)

- Opt-in second aggregation over the same `git log` pass: unordered file pairs that change in the same commit
...
- Emitted only in the JSON envelope (`coupling` section, additive to schema v1); combining `--coupling` with table/csv is an error
...
| # | Goal | Status |
| --- | --- | --- |
| 1 | Change coupling analysis | **Shipped** as `hc analyze --coupling` + `hc annotate` partner notices (proposal 012) |
