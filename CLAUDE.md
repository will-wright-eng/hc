...
cmd/hc/main.go          CLI entry (urfave/cli v3). Subcommands: analyze, md (report,
                        ignore), annotate. Root command shares analyze's flags + Action
...
internal/md/             Markdown renderers: report.go (analysis JSON → markdown, with
                         ...
                         (LLM prompt for .hcignore generation). Templates in templates/.
internal/annotate/       Renders the analyze envelope as GitHub Actions workflow-command
                         annotations (::warning/::notice) for changed hotspot files —
                         level by quadrant, diff-line anchoring via AnchorLines. Consumes
                         schema.Envelope like internal/md.
```
...
- **Change coupling** (`--coupling`): opt-in, JSON-only `coupling` envelope section of same-commit file pairs — raw support, decay-weighted support, asymmetric confidences. Fixed floors (support ≥ 5, max confidence ≥ 0.5; a documented departure from median-split). Commits listed in `.git-blame-ignore-revs` at the repo root are skipped for pair extraction (churn unaffected). Combining `--coupling` with table/csv is an error. `hc annotate` emits "missing co-change partner" `::notice`s for pairs where the PR touches exactly one side. See `docs/proposals/012-change-coupling.md`.
